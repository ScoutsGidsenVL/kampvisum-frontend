...
export class ParticipantRepository extends BaseRepository {
  ...
  search(groupId: string, query: string, filter?: Filter): Promise<any> {
    return this.get(groupId, `${this.endpoint}?term=${query}${filter && filter.gender ? '&gender=' + filter.gender : ''}${filter && filter.ageMin ? `&min_age=${filter.ageMin}` : ''}${filter && filter.ageMax ? `&max_age=${filter.ageMax}` : ''}${filter && filter.type ? `&type=${filter.type}` : ''}`, {}).then((response: any) => {
      const array: any[] = []
