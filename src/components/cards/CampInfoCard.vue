    ...
    <div @click="navigateTowardsVisum(visum)" class="p-3 cursor-pointer shadow-md rounded-md hover:bg-lighterGreen">
      <div class="z-2">
        ...
        <div v-for="category in visum.categorySet.categories" :key="category" class="d-flex gap-3 my-2.5 items-center">
          ...
          <h4 @click.stop="navigateTowardsCategory(category.categoryParent.name, visum, category.id, route)" class="hover:underline mb-0 ml-0 xs:text-base md:text-md" >{{ category.categoryParent.name }}</h4>
        </div>
      </div>
    </div>
</template>
