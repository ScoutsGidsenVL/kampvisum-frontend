<template>
  <div class="bg-lighterGreen md:z-20 md:mt-3"
    :class="{ 'xs:w-full xs:fixed xs:top-0 xs:right-0 xs:h-full': sidebar.state === SidebarState.OPEN, 'w-8 d-flex': sidebar.state === SidebarState.CLOSED, 'xs:z-50': sidebar.state === SidebarState.OPEN }">
    <!-- WHEN CLOSED -->
    <div @click="openSideBar()" class="w-8 h-screen-90 fixed flex-column pt-7 cursor-pointer top-custom"
      :class="{ 'd-none': sidebar.state === SidebarState.OPEN, 'd-flex': sidebar.state === SidebarState.CLOSED }">
      <div class="flex justify-center w-100">
        <svg style="margin-top: 10px; margin-left: 4px" class="mr-1" width="20" height="20" viewBox="0 0 20 20"
      ...
      <div class="h-screen flex flex-wrap">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 sticky mt-20" fill="none" viewBox="0 0 24 24"
          stroke="currentColor">
    ...
    <div style="margin-bottom: -10000px;" class="sticky h-screen"
      ...
      <div>
        <div class="flex justify-between">
          ...
          <svg @click="closeSideBar()" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
            class="cursor-pointer bi bi-x-lg stroke-current stroke-1 text-black" viewBox="0 0 16 16">
            <path
