<template>
    <div>
        <div v-for=" call in calls" class="grid grid-cols-5 py-2 gap-2">
            <h4>{{ call }}</h4>
            <input :placeholder="index" class="p-2 border-2" v-for="(variable, index) in callsObject[call].vars"
                v-model="callsObject[call].vars[index]" type='text' />
            <button class="bg-black text-white py-2 text-center"
                @click="callsObject[call].call(callsObject[call].vars)">call</button>
        </div>
    </div>
    ...
    <div v-for="vartype in varTypes" class="grid grid-cols-5 py-2 gap-2">
        ...
        <button class="bg-black text-white py-2 text-center"
            @click="overWriteVars(vartype.label, vartype.value)">Overwrite</button>
    </div>
</template>
